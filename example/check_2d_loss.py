...
def main():
    ...
    # load the model
    state_dict = torch.load("/netscratch/nafis/human-pose/new_code_to_git/stacked-human-pose/results/stgh4_mgcn2_5fps_schedul/model_41.pth")
    ...
    if wandb_flag:
        wandb.login()
        wandb.init(project="check_loss", entity="nafisur")
    ...
    for epoch in range(epochs):
        ...
        if valid_loss < best_loss:
            if save_checkpoint:
                result_path = Path(f'results/trash/model_diff_{epoch}.pth')
...
def validate(val_loader, model, criterion, num_classes, epoch, debug=False, flip=True):
    ...
    with torch.no_grad():
        ...
        bar.close()
    return losses.avg, predictions, acces.avg
