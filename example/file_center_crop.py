...
model = MyNet(adj,num_stacks =2, block=2) # 2 stacks and 2 gcn blocks
...
for file in directory.iterdir():
    ...
    x1 = int((width - target_size[0]) / 2)
    y1 = int((height - target_size[1]) / 2)
    x2 = x1 + target_size[0]
    y2 = y1 + target_size[1]
    img = img[y1:y2, x1:x2]
    crop_img = img_2[y1:y2, x1:x2]
    ...
    for pts in key_points:
        cv2.circle(crop_img, (int(pts[0]), int(pts[1])), 3, (0, 0, 255), -1)
        # draw the skeleton for the points
    # for connection in skeleton:
    #     cv2.line(crop_img, (int(key_points[connection[0], 0]), int(key_points[connection[0], 1])),
    #             (int(key_points[connection[1], 0]), int(key_points[connection[1], 1])), (255, 0, 0), 2) 
