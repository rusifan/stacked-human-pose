...
from net_without_softmax import MyNet
...
def main():
    ...
    # lr = 2.5e-4
    lr = 2.5e-5
    # lr = 0.00000750 # for continued training 2stg
    # schedule = [60, 90]
    schedule = [ 10, 15, 30, 50, 60, 90]
    # gamma = 0.1
    gamma = 0.3
    sigma_decay = 0
    njoints = 16
    ...
    # model = MyNet(adj, block=1) # 1stack and 2 gcn
    # model = MyNet(adj, block=2, num_stacks=8) # 8stack and 2 gcn
    # model = MyNet(adj, block=2, num_stacks=2) # 8stack and 2 gcn
    model = MyNet(adj, block=2, num_stacks=4) # 4stack and 4 gcn
    model = torch.nn.DataParallel(model).cuda()
#     import pdb;pdb.set_trace()
# # load the model
    # state_dict = torch.load("/netscratch/nafis/human-pose/new_code_to_git/stacked-human-pose/results/stgh2_mgcn2_nosoft_deeper_gcn/model_deep_30.pth")
    # state_dict = torch.load("/netscratch/nafis/human-pose/new_code_to_git/stacked-human-pose/results/stgh2_mgcn2_nosoft_deeper_gcn/model_deep_30.pth")
    ...
    if wandb_flag:
        wandb.login()
        wandb.init(project="trash", entity="nafisur")
    ...
    for epoch in range(epochs):
        ...
        if valid_loss < best_loss:
            # result_path = Path(f'results/stgh4_mgcn2_nosoft_4_gcn/model_deep{epoch}.pth')
