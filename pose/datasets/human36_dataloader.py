...
class hum36m_dataloader(Dataset):
    def __init__(self, data_set_path, annotaion_path, use_crop, scale_range, use_flip, min_pts_required,
                 pix_format='NHWC', normalize=True, flip_prob=0.3):
    ...
    def __getitem__(self, index):
        ...
        ratio_x = 1 / image.shape[0]  # image.shape[0]
        ratio_y = 1 / image.shape[1]  # image.shape[1]
        # kps[:, :1] *= ratio_x 
        # kps[:, 1:2] *= ratio_y
        kps[:, 0] = kps[:, 0] * ratio_x
        kps[:, 1] = kps[:, 1] * ratio_y
        ...
        dst_image = color_normalize(dst_image, self.mean, self.std)

        # self._show(dst_image, heatmaps)
    ...
    def _show(_image, _heatmap):
        ...
        if _heatmap is not None:
            ...
            plt.subplot(1, 2, 1)
            plt.imshow(np.sum(_heatmap, axis=0))
